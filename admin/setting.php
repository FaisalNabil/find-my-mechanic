...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
   ...
   $jsonShopOwnerString = getJSONFromDB("SELECT Password FROM admin WHERE Email='".$_SESSION["adminEmail"]."'");
                ...
                $error = 
                 '<div class="alert alert-danger alert-dismissable">
                   ...
                   <strong>Worning!</strong> Password must be at least 5 characters and must contain at least one lower case letter, one upper case letter and one digit!.  
                    ...
                    $sql = "UPDATE admin SET Password ='".$newPassowrd."' WHERE Email='".$_SESSION["adminEmail"]."'"; 
                    $resql="UPDATE carshop SET Password ='".$newPassowrd."' WHERE Email='".$_SESSION["adminEmail"]."'";
                    if (updateDB($sql)==1) {
                      updateDB($resql);
